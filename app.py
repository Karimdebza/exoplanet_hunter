...
def analyze_star(star_name):
    ...
    try:
        ...
        # Remplace la boucle par ça
        segments = []
        indices = []

        for i in range(0, len(flux) - window, step):
            segments.append(flux[i:i+window])
            indices.append(i)

        # Une seule prédiction sur tout d'un coup
        X_batch = np.array(segments).reshape(-1, window, 1)
        probas = model(X_batch, training=False).numpy().flatten()
