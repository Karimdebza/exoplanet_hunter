...
def _is_alias(period_a: float, period_b: float) -> bool:
    ...
    # Cherche si ratio est proche d'un rationnel simple n/m
    # avec n,m petits entiers (1 à 8 suffit en pratique)
    for n in range(1, 9):
        for m in range(1, 9):
            if abs(ratio - n/m) / (n/m) < ALIAS_TOLERANCE:
                return True
    return False
...
def detect_signals(
    ...
    # GRILLE EN FRÉQUENCE — pas en période.
    # np.linspace(period_min, period_max) = erreur classique.
    # Le BLS a besoin d'une résolution δP ∝ P², donc δf = constante.
    # Espacement uniforme EN FRÉQUENCE = bien échantillonner les courtes périodes.
    freq_min = 1.0 / period_max
    freq_max = 1.0 / period_min
    freq_grid = np.linspace(freq_min, freq_max, n_periods)
    period_grid = 1.0 / freq_grid[::-1]  # tri croissant pour astropy
